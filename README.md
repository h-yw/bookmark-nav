...
- 本地数据管理：导出/导入设置、常用/最近记录、标签和操作历史。
...
导出/导入只处理扩展本地设置、常用/最近记录、标签和操作历史，不导出浏览器书签树，也不会修改浏览器书签。
...
- 设置、常用/最近记录、标签、操作历史和最近一次检测记录保存在扩展本地。
...
不会。导入/导出只处理扩展本地设置、常用/最近记录、标签和操作历史，不读写浏览器书签树。
