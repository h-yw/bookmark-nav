...
- 本地标签：给书签添加独立于浏览器文件夹的标签，并按标签筛选。
- 工作区：把文件夹、标签和搜索词组合成场景入口。
- 书签管理：支持复制链接、编辑、移动、删除，以及批量复制、批量移动、批量删除。
- 整理报告：检查重复链接、相似链接、空文件夹、标题异常、长期未打开和疑似失效链接。
- 本地数据管理：导出/导入设置、常用/最近记录、标签、工作区和操作历史。
...
工作区可以把常用文件夹、标签和搜索词保存成一个入口。适合按场景整理书签，例如“前端开发”“项目 A”“学习资料”。

常见用法：

- 在某个文件夹中点击“创建工作区”，快速保存当前文件夹。
- 在某个标签筛选中点击“创建工作区”，快速保存当前标签。
- 在工作区管理中组合多个文件夹、标签和搜索词。

工作区只影响 Bookmark Nav 的本地筛选视图，不会修改浏览器书签树。
...
导出/导入只处理扩展本地设置、常用/最近记录、标签、工作区和操作历史，不导出浏览器书签树，也不会修改浏览器书签。
...
- 设置、常用/最近记录、标签、工作区、操作历史和最近一次检测记录保存在扩展本地。
...
不会。导入/导出只处理扩展本地设置、常用/最近记录、标签、工作区和操作历史，不读写浏览器书签树。

**标签会修改浏览器书签吗？**

不会。标签只保存在扩展本地数据中，独立于浏览器原生文件夹。

**工作区会修改浏览器书签吗？**

不会。工作区只是扩展本地的筛选入口，用来组合文件夹、标签和搜索词。

**操作历史会记录所有修改吗？**

不会。当前主要记录批量删除和批量移动。单个删除、编辑等操作仍需要谨慎确认。
...
- [阶段三体验审查](docs/stage-three-review.md)
