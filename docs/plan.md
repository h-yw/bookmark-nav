# Bookmark Nav 开发计划

基于 `roadmap.md` 阶段规划和当前代码进展，记录各阶段任务和优先级。

## 当前版本：v1.1.1

MVP 功能全部完成，阶段二健康检查主要功能已完成。当前重点转入 v1.2 发布准备和稳定性收口。

---

## 阶段一：稳定发布版（~90%）

核心功能已完成，剩余发布准备工作。

| 任务 | 状态 | 备注 |
|------|------|------|
| Chrome MV3 构建 | ✅ | |
| Edge 支持 | ✅ | |
| Firefox 构建 | ✅ | |
| CI/CD 自动发布 | ✅ | semantic-release |
| 搜索排序优化 | ✅ | 标题/域名/URL/路径权重 |
| 搜索历史提升 | ✅ | 常用/最近加权 |
| 测试覆盖 | ✅ | 106 个测试通过 |
| manifest 权限检查 | ✅ | check-release-manifest.cjs |
| 工程结构整理 | ✅ | WXT `srcDir: 'src'`，按 entrypoints/components/domain/storage/shared 分层 |
| 仓库 description/topics/license | 🟡 | MIT License 已补，GitHub description/topics 待在仓库设置中配置 |
| README 产品截图 | 🟡 | 已补截图占位，待替换真实截图 |
| Chrome/Edge 商店截图 | ❌ | 待补 |
| 用户文档 | ✅ | 安装、权限、FAQ、Release 产物说明已覆盖 |

---

## 阶段二：书签健康检查（~90%）

| 任务 | 状态 | 备注 |
|------|------|------|
| 重复链接检测 | ✅ | BookmarkReport |
| 空文件夹检测 | ✅ | BookmarkReport |
| 标题异常检测 | ✅ | BookmarkReport |
| 长期未打开检测 | ✅ | BookmarkReport |
| 相似链接检测 | ✅ | 同一域名 3+ 书签归组 |
| 操作快照备份 | ✅ | 批量删除/移动前自动创建 |
| 快照恢复 | ✅ | 含备用文件夹 |
| 重复书签处理交互 | ✅ | DuplicateBookmarksDialog |
| 报告中跳转文件夹 | ✅ | 文件夹路径可点击 |
| 最近检测记录 | ✅ | 保留最近一次失效链接检测记录并在报告展示 |
| 报告链接跳转 | ✅ | 报告中的书签链接可点击打开 |
| 导出清理报告 | 🟡 | 函数保留，按钮暂隐藏 |
| 失效链接检测 | ✅ | no-cors 模式，手动触发 |

---

## v1.2 发布准备

目标：把当前功能整理成可公开发布的稳定版本，优先补齐文档、授权、截图和发布检查闭环，不引入大范围新功能。

| 任务 | 状态 | 备注 |
|------|------|------|
| MIT License | ✅ | `LICENSE` + `package.json` metadata |
| README 安装路径 | ✅ | Chrome/Edge/Firefox 手动加载说明 |
| README 权限解释 | ✅ | bookmarks-only，无 content_scripts |
| README FAQ | ✅ | 数据、favicon、导入导出、操作历史边界 |
| README 与原生管理器区别 | ✅ | 明确使用边界 |
| README 发布产物说明 | ✅ | zip 和 checksums |
| 发布检查清单 | ✅ | `docs/release-checklist.md` |
| 真实产品截图 | ❌ | 待用本地构建或商店素材补齐 |
| 商店描述文案 | ✅ | `docs/store-listing.md` |
| 仓库 description/topics | ❌ | 需要在 GitHub 仓库设置中配置 |

---

## 阶段三：书签管理器

| 任务 | 状态 | 备注 |
|------|------|------|
| 操作历史与撤销 | 🟡 | 批量删除/移动已支持操作历史和最近一次撤销；单个操作待扩展 |
| 标签系统 | ✅ | 编辑、批量追加、管理、筛选、导出/导入 |
| 阶段三体验审查 | ✅ | `docs/stage-three-review.md` |
| 批量改名 | ⏸️ | 暂缓，使用场景偏低频 |
| 收件箱 | ⏸️ | 暂缓，待确认快速临时保存是否为核心场景 |
| 文件夹管理 | ⏸️ | 暂缓，避免过早进入复杂管理后台 |

---

## 阶段四：工作区与命令面板

| 任务 | 状态 | 备注 |
|------|------|------|
| 工作区 | 🟡 | 第一版：组合入口 + 快速创建 + 摘要展示 |
| 命令面板 | ❌ | 下一步建议：只做打开和切换，不做危险管理动作 |
| 自定义搜索引擎 | ❌ | GitHub/npm/MDN 等 |
| 项目入口聚合 | ❌ | 聚合相关链接 |

---

## 阶段五：AI 辅助整理

待阶段三/四成熟后评估。

---

## 阶段六：同步与备份

待阶段五成熟后评估。

---

## 版本节奏建议

- `1.1.x`：阶段二收尾（失效链接检测、检测记录、工程结构迁移）
- `1.2.x`：发布准备（License、README、发布检查清单、截图/商店素材）
- `1.3.x`：标签系统 + 工作区 + 阶段三体验收口
- `1.4.x`：命令面板 + 自定义搜索
- `2.0.x`：AI 辅助或同步（如果引入）
